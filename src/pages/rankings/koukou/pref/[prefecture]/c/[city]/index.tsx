      ...
      <Container maxWidth="lg" sx={{ py: { xs: 2, sm: 3 } }}>
        ...
        <Box sx={{ mb: 4, p: 2, bgcolor: "#F5F5F5", borderRadius: 1 }}>
          ...
          <Typography variant="body2" sx={{ lineHeight: 1.8, color: "#424242" }}>
            学校選びの際には、偏差値だけでなく、教育方針、部活動、施設、アクセスなど、多角的な観点から情報を検討することをお勧めします。
          </Typography>
        </Box>
      </Container>
    </>
...
export const getServerSideProps: GetServerSideProps<CityRankingPageProps> = async ({ params, res, query }) => {
  const prefSlug = params?.prefecture as string;
  ...
  const { data: cityData } = await supabase.from("City").select("name").eq("id", cityId).single();
